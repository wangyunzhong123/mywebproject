...
jQuery(document).ready(function() {
	//设置默认选中
	$("[data-selected='true']").val($("[data-selected='true']").data('value'));
});
...
$('.digitalDetailsView').data('countToOptions', {
...
// @group  指定数字指定时间 进行动画  end


// @ .banneralert.ui-banneralert
function banner_alert(content, type) {
	type = type || "warning";
	var addclass = "ui-banneralert ";
	addclass = addclass += type;
	$("body").showbanner({
		content: content,
		duration: 1500,
		addclass: addclass
...
{
    if ($("body").find('[id^="tag_cloud-"]').length >= 1) {
        $tagid = $("body").find('[id^="tag_cloud-"]').attr('id');
    	var i=0;
    	var oTag=null;
    	oDiv=document.getElementById($tagid);
		...
		render: function() {
			...
			for ( var list in social ) {
				...
				if ( typeof data !== "undefined" ) {
					arrayData.push( '<a href="javascript:" data-type="'+ list +'" class="btn-share btn-share-'+ data.className +'" title="分享到'+ data.text +'"><i class="uicon-share-'+ data.className +'"></i></a>' );
				}
...
;$(document).ready(function() {
	...
	$(window).scroll(function() {
		if ($(this).scrollTop() > 100) {
			$(".widget_fixed_contact .c_meau").stop().show().animate({
				bottom: '100px'
			}, 300);
		} else {
			$(".widget_fixed_contact .c_meau").stop().animate({
				bottom: '-430px'
			}, 300);
		}
	});
	...
	if (true === $is_pc) {
		...
		$('#navbar .service-list').hover(function() {
			...
			_self.addClass('on').find('>ul.sub-menu').fadeIn(300);
			//}, 100)
		}, function() {
			//clearTimeout(tw);
			//tf = setTimeout(function() {
			_self.removeClass('on').find('>ul.sub-menu').fadeOut(300);
			//}, 100)
		});
	...
	$(".ui-account").on("mouseover", function() {
	...
	//显示隐藏更多服务
	$(".show-hide-service").click(function(event) {
		$is_hidden = $(".hide-service").data('hidden');
		//打开服务列表
		if (true === $is_hidden) {
			$(".hide-service").removeClass('hidden').data('hidden', false);
			$(".show-hide-service.top").addClass('hidden');
			$(".show-hide-service.bottom").removeClass('hidden');
			slow_moving("more-services");
		} else {
			$(".hide-service").addClass('hidden').data('hidden', true);
			$(".show-hide-service.top").removeClass('hidden');
			$(".show-hide-service.bottom").addClass('hidden');
			slow_moving("ui-service-list");
		}
	});
	...
	jQuery(".messageScroll_slide").slide({
		mainCell: ".bd ul",
		autoPlay: true,
		effect: "topMarquee",
		vis: 6,
	...
	jQuery(".qb").fancybox({
		...
		'closeEffect': 'fade',
		'afterLoad': function() {
			if ($(this.element).hasClass('reg')) {
				switch_active("signupActive");
			} else {
				switch_active("login");
			}
		}
		...
		if ($(this).hasClass('done')) {
			...
			$.post("/wp-admin/admin-ajax.php", ajax_data,
				function(data) {
					...
					$('.blogLike').poshytip('update', "喜欢 (<span class='likenum'>" + $.trim(data) + "</span>) ");
				});
			return false;
		}
	};
	...
	//提交评价
	$(".SuggestionsBtn_ok").click(function() {
		var letterTitle = $(".js-zendesk-tag").val();
		var receiverName = "6066476@qq.com";
		var messageContent = $(".js-zendesk-content").val();
		var fromUser = $(".jyyzxMail").val();
		var fromUserName = $(".jyyzxName").val();
		var currentUrl = $(".currentPathByjjyzx").val() + "/m/letterEdit.php";
		//Title
		dataStr = "action=sendLetter&fromUser=" + fromUser + "&fromUserName=" + fromUserName + "&letterTitle=" + letterTitle + "&receiverName=" + receiverName + "&messageContent=" + messageContent + "&ran=" + Math.random();

		$(".SuggestionsContent").hide();
		$(".jytsLoad").show();

		//提交load
		$(".SuggestionsBtn_ok").addClass("jytsLoadButton").removeClass("SuggestionsBtn_ok");
		$.ajax({
			url: currentUrl,
			type: "POST",
			dataType: "json",
			...
			success: function(m) {
				...
				$(".jytsLoad").hide();
				$(".jytsTip").show();
				setTimeout('closeJyyzx()', 1000);
			}
		});
	});

	//分享至SNS
	jQuery(document).on("click", ".shareto", function() {
		var t = $(this).attr("data-sharetoSNS");
		h($(this), t);
		return false
	});

	//为首页banner链接加上跳转功能
	$(".DB_bgSet li").click(function() {
	...
	//登录表单提交
	jQuery("form#loginform, form#signupform").on("submit", function() {
		_this = jQuery(this);
		_btn_text = jQuery(".submit", _this).val();
		_title_text = jQuery("h4", _this).text();
		if (_this.valid()) {
			jQuery("h4", _this).text("正在请求中，请稍等...");
			ajax_data = _this.serializeArray();
			jQuery(".submit", _this).attr("disabled", "disabled").addClass("disabled").val("注册中...");
			jQuery.post('/wp-admin/admin-ajax.php', ajax_data,
				function(data) {
					// 后台验证错误
					if (data.status == -1) {
						banner_alert(data.message);
						jQuery(".submit", _this).removeAttr('disabled').removeClass('disabled').val(_btn_text);
						jQuery("h4", _this).text(_title_text);
					} else {
						if (data.status == 1) window.location.href = jQuery(".redirect_to", _this).val();
					}
				}
			);
			return false;
	...
	//重发激活信
	jQuery("#resendActivationLetter").on("click", function() {
		_this = jQuery(this);
		action = "uiresendmail";
		...
		jQuery.ajax({
			...
			success: function(b) {
				ui_gritter("success", b.message);
			},
			complete: function() {
				jQuery("form#loginform .submit").removeAttr("disabled").removeClass("disabled").val("登录"), jQuery("form#signupform .submit").removeAttr("disabled").removeClass("disabled").val("注册")
			}
		});
	})
	...
	$(".paypay-form .btn-paypal").click(function() {
		...
		$("input[name='itemName']").val($(".order-tit").text());
		...
		_self.attr('disabled', true).text("订单处理中");
		$.ajax({
			type: "post",
			url: '/wp-admin/admin-ajax.php',
			data: ajax_data,
			...
			success: function(data) {
				if (data.pstate == "y") {
					...
					_self.removeAttr('disabled').text("进入PayPal付款");
					pform = data.pform;
					$form = $(pform).appendTo("body")
					$form.submit();
				}
			}
		});
	...
	if ($("#share_box").length > 0) {
		$('#share_box').css('left', $("body.single #primary")[0].offsetLeft - 45.5);
	}
	...
	$('#share_box').share({
		...
		social: [ 'weibo','qq',/*'qzone', 'tqq', 'renren', 'weixin', 'qq','twitter'*/]
	});
...
function initSelectedMenu() {
	...
	//判断是否是订单详情页面  特别设置
	if (strPage == "m-orderdetails") {
		$("#Default").parent().addClass("online");
	}
	if (strPage == "m-transactiondetails") {
		$("#transactiondetails").parent().addClass("online");
	}
	if (strPage == "m-order") {
		$("#order-list").parent().addClass("online");
	}


	//资讯页面选中
	$('.samenu li a').each(function() {
		$curMenu = $("#cmenu").val();
		$curUrl = $($(this))[0].href;
		$num = $curUrl.indexOf($curMenu);
		if ($num > 0) {
	...
	//资讯页面子类选中
	$('.inHd li a').each(function() {
		$subcatMenu = $("#subcat").val();
		if ($subcatMenu != "subcat=") {
			$curUrl = $($(this))[0].href;
			$num = $curUrl.indexOf($subcatMenu);
			if ($num > 0) {
				$(this).parent().addClass('on').append('');
				index = 0;
			}
		}
	});

	//page slider for page
	jQuery(".CarouselSlide").slide({
		mainCell: ".bd ul",
		autoPlay: true,
		trigger: "click"
	});
...
if ($("body").hasClass("home")) {
	(function() {
		var hotWordCtn = $('.hot-comment');
		var titleStr = KeywordConfig.title;
		var curIndex = 0;
		...
		function showHotWord() {
			...
			for (var i = curIndex; i < KeywordConfig.num + curIndex; i++) {
				if (KeywordConfig.data[i]) {
					html += '<span class="comment-name">' + KeywordConfig.data[i].name + '：</span><span>' + KeywordConfig.data[i].comment + '</span>';
				}
			}
			...
			for (var i = 0; i < children.length; i++) {
				$(children[i]).delay(100 * i).animate({
					opacity: 0.1
				}, 200, function() {
					$(this).animate({
						opacity: 1
...
//切换表单
function switch_form($op) {
	switch_active($op);
	var window_h = $(".fancybox-wrap").height();
	var w_h = $(window).height();
	var top = (w_h - window_h) / 2;
	$(".fancybox-wrap").css({
		"top": top
	});
}
...
//切换登录注册
function switch_active($op) {
	if ($op == "signupActive") {
		jQuery("#loginform").addClass('hidden');
		jQuery("#signupform").removeClass('hidden');
	} else {
		jQuery("#signupform").addClass('hidden');
		jQuery("#loginform").removeClass('hidden');
	}
}

//跳转锚点
function slow_moving($id) {
	location.hash = $id;
}
